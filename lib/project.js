...
console.log(this);
    ...
    async refreshAPIVersions() {
        this.connection.versions = await RestAPI.getApiVersions(this.connection.host).send();
        return this.save();
    }

    async refreshUser() {
        return RestAPI.getUserInfo(this.connection.id).send(this);
    }

    async refreshFromServer() {
        try {
            const response = await MetadataAPI.retrieve(this.connection.baseurl, this.apiVersion, await this.getToken(), this.packageXml);
            this.metadata = response.files;
            this.save();

            const fileWrites = Object.entries(response.zip.files).map(async ([dir, zipObj]) => {
    ...
    async authenticate(type) {
        ...
        const [[userid, username], versions] = await Promise.all([
            RestAPI.getUserInfo(result.id, result.token_type + " " + result.access_token).send(),
            RestAPI.getApiVersions(host).send()
        ]);
    ...
    async reauthenticate() {
        ...
        const result = await RestAPI.refreshToken(host, await refreshKey).send();
