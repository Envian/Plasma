...
class ToolingRequest {
    ...
    async send(project) {
        ...
        try {
            const [result, statusCode] = await rest_request_js_1.sendAuth(project, this.options, this.body);
    ...
    async sendRaw(project) {
        this.options.path = `/services/data/v${project.apiVersion}/tooling` + this.options.path;
        try {
            const [result, statusCode] = await rest_request_js_1.sendAuthRaw(project, this.options, this.body);
            this.handleResponse(result, statusCode);
            return result;
        }
        catch (error) {
