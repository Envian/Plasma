...
async function saveTooling(project, files) {
    ...
    let saveRequests = Object.entries(files).map(([name, savedFiles]) => {
        switch (name.substr(0, name.indexOf("/"))) {
            case "classes":
            case "triggers":
                return new ApexSave(name, savedFiles);
            case "components":
            case "pages":
                return new VisualforceSave(name, savedFiles);
            case "arua":
                // TODO
    ...
    const conflictQueries = await Promise.all(saveRequests.map(save => save.getConflictQuery(project)));
    await new ToolingAPI.CompositeRequest({ project, requests: conflictQueries }).send(project);
    await Promise.all(saveRequests.map(request => request.checkConflicts(project)));
    saveRequests = saveRequests.filter(file => !file.skip);

    if (saveRequests.length) {
        const deploy = new ToolingAPI.CompositeRequest({ project });
        ...
        deploy.addAll(saveRequests.map(save => save.getSaveRequest(project, "MetadataContainer")));
        ...
        const status = await Promise.all(saveRequests.map(request => request.handleSaveResult(project, resultsByFile[request.source.path])));
